...
float parMaisProx(int n, P pontos[], P* pMin1, P* pMin2){
    ...
    if (n <= 3){
        ...
        for(int i = 0; i < n; i++){
            for(int j = i+1; j < n; j++){
                float dist = dis(pontos[i], pontos[j]); //                                O(6)
                if (dist < min){
                    min = dist;
                    *pMin1 = pontos[i];
                    *pMin2 = pontos[j];
    ...
    float minEsq = parMaisProx(meio, pontos, &pEsq1, &pEsq2);         // O(n/2) 
    float minDir = parMaisProx(n-meio, pontos+meio, &pDir1, &pDir2);  // O(n/2)
    ...
    for(int i = 0; i < n; i++){
        if (fabs(pontos[i].x - pMeio.x) < min){ 
            faixa[cont] = pontos [i]; //                                            O(n)
            cont++;
        } 
    }
    
    // Encontrar o mais proximo dentro da faixa
    MergeSortY(faixa, 0, cont-1); // ordena a faixa com relacao a Y                 O(n log n)
