...
int verifica(int linha, int coluna, int tabuleiro[]){ // funcao que verifica se a posiçao do tabuleiro é válida ----
    ...
    // verifica a coluna ----------------------------------------
    for (int l = 0; l < linha; l++) // l de 0 até linha, apenas as linhas anteriores   O(n)
    ...
    int c = coluna-1;
    for (int l = linha-1; l >= 0; l--){ // l de linha-1 até 0, na ordem inversa  O(n)
...
void rainha(int tabuleiro[], int n, int linha){ // funcao recursiva que posiciona uma rainha em uma linha, imprime a solução caso o tabuleiro esteja completo
    ...
    for (int coluna = 0; coluna < n; coluna++){
        if(verifica(linha, coluna, tabuleiro)){
            ...
            rainha(tabuleiro, n, linha+1); // chama a funcao para a próxima linha                                           //O(n!)

            tabuleiro[linha] = -1; // após a chamada recursiva, remove a rainha para testar novas soluções
        }
    }
    //Complexidade = O(n!)
}
...
int main(){
    ...
    printf("     Pedro Henrique Souza e Yago Campello\n\n");
    ...
    printf("Tempo total de execucao: %fs", tempoTotal);
